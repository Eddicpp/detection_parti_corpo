#!/usr/bin/env python3
"""Train YOLO on local machine using CUDA (e.g. GeForce RTX 3060)."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train YOLO on local CUDA GPU")
...
def main() -> int:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError(
            "CUDA non disponibile. Verifica driver NVIDIA e installazione PyTorch con supporto CUDA."
        )
